variable "project_name" {
  type        = string
  description = "Overall name of the project/solution used to name resources in AWS."
  default     = "trailwatch"
}
...
variable "secgroup_automation_principal_exceptions" {
  type        = list
  description = "List of principals that will be given exception when opening sensitive INGRESS ports in the organization. This can either be an IAM username or a Role name."
}

variable "secgroup_automation_monitored_ports" {
  type        = list
  description = "List of ports that when opened to 0.0.0.0/0 in Security Groups will be automatically deleted in the organization."
  default     = [ 22, 23, 389, 445, 1433, 3306, 3389, 5432, 6379, 11211 ]
}
